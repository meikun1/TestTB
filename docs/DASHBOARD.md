...
Sender держит фоновую задачу, которая раз в `POOL_RELOAD_INTERVAL_SECONDS`
(по умолчанию 300с) сверяет пул с БД:

- новые intake-аккаунты подключаются и сразу попадают в ротацию
- забаненные/disabled — отключаются, их клиенты закрываются
- если у аккаунта сменились `session_string`/`session_path`/`proxy` —
  клиент пересобирается с новыми параметрами
- остальные изменения (лимиты, статус active←flood и т.п.) применяются
  без пересоздания клиента

Чтобы выключить live-reload и работать только со снапшотом старта —
поставь `POOL_RELOAD_INTERVAL_SECONDS=0` в `.env`. Тогда новые аккаунты
попадут в пул только после рестарта sender'а.
