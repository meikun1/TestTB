...
**Глобальный тумблер `PROXIES_ENABLED` в `.env`.** Если `false` — поле
`Account.proxy` игнорируется и все аккаунты ходят напрямую с IP сервера.
Удобно для теста на одном-двух аккаунтах. На реальном масштабе включай
в `true` и заполняй прокси в CSV — без этого Telegram быстро кластеризует
всех «соседей по IP» и забанит каскадом.
